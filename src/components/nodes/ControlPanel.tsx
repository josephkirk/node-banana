        ...
        <div className="p-4">
          ...
          <div className="space-y-4 mt-4">
            ...
            {selectedNode.type === "subflow" && (
              <SubFlowControls node={selectedNode} />
            )}
          </div>
        </div>
      </div>
...
// Subflow Controls
function SubFlowControls({ node }: { node: Node }) {
  const nodeData = node.data as SubFlowNodeData;
  const updateNodeData = useWorkflowStore((state) => state.updateNodeData);
  const exportSubFlow = useWorkflowStore((state) => state.exportSubFlow);
  ...
  return (
    <div className="space-y-4">
      <div>
        <label className="block text-xs font-medium text-neutral-300 mb-1">Subflow Name</label>
        <input
          type="text"
          value={nodeData.name || ""}
      ...
      <div className="flex flex-col gap-2">
        <button
          onClick={() => diveIn(node.id)}
          className="nodrag nopan w-full px-3 py-2 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold rounded transition-colors flex items-center justify-center gap-2"
        >
        ...
        <button
          onClick={() => exportSubFlow(node.id, nodeData.name || "subflow")}
        ...
        {nodeData.externalPath && (
          <div className="p-3 bg-neutral-900/50 rounded-lg border border-neutral-700 space-y-2">
            <div className="text-[10px] text-neutral-500 uppercase tracking-widest font-bold">External File</div>
            <div className="text-xs text-neutral-300 font-mono truncate">{nodeData.externalPath}</div>
            
            <button
              onClick={() => toggleSubFlowLink(node.id)}
              className={`w-full px-3 py-1.5 rounded text-[10px] font-bold uppercase tracking-wider transition-colors ${
                nodeData.isLinked 
                  ? "bg-amber-500/20 text-amber-500 border border-amber-500/40 hover:bg-amber-500/30" 
                  : "bg-neutral-800 text-neutral-400 border border-neutral-700 hover:bg-neutral-700"
              }`}
            >
              {nodeData.isLinked ? "LINKED TO FILE" : "STAY INLINE"}
            </button>
          </div>
        )}
      </div>
